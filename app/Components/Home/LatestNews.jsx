    ...
    <div className="bg-gradient-to-br from-gray-50 to-white py-20 overflow-hidden relative">
      ...
      <div className="container mx-auto">
        ...
        <div className="container">
          {/* Header */}
          <div
            className={`text-left mb-16 transition-all duration-1000 transform ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
              }`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {newsItems.map((item, index) => (
              <div
                key={item.id}
                className={`relative group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-700 transform hover:-translate-y-3 overflow-hidden cursor-pointer ${isVisible
              ...
              >
                ...
                <div className="relative overflow-hidden h-64">
                  <img
                    src={item.image}
                    alt={item.title}
                    className={`w-full h-full object-cover transition-transform duration-700 ${hoveredCard === item.id ? "scale-110" : "scale-100"
                      }`}
                  />

                  {/* Category */}
                  <div
                    className={`absolute top-4 left-4 px-3 py-1 bg-red-500 text-white text-xs font-semibold rounded-full transform transition-all duration-500 ${hoveredCard === item.id
